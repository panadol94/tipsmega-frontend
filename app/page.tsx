      ...
      <header className="card relative overflow-hidden flex flex-col min-h-[300px] justify-between p-0 group border-amber-500/20">
        ...
        <div className="relative z-10 p-5 mt-auto bg-gradient-to-t from-black to-transparent pt-10">
          ...
          {!isLoggedIn ? (
            ...
            <div className="flex gap-3 transform-gpu isolate">
              <button
                onClick={claimNow}
                disabled={isClaiming}
                className="btn-ghost backdrop-blur-md hover:bg-purple-500/20 flex-1 h-[52px] border-purple-500/30 bg-purple-500/10 disabled:opacity-50 disabled:cursor-not-allowed transform-gpu"
              >
                <span className="font-black text-sm">{isClaiming ? '⏳ CLAIMING...' : '✨ CLAIM STARS'}</span>
              </button>
              <button
                className="btn-ghost backdrop-blur-md hover:bg-white/10 h-[52px] px-4 border-white/20 transform-gpu"
                onClick={() => {
                  if (confirm("Log out?")) {
