        ...
        <div className="max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    ...
                    <p className="text-slate-500 text-sm">Add a game to the scan pool</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-slate-800 border border-slate-700 rounded-2xl p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                            type="number"
                            value={form.rtpMin}
                            onChange={(e) => setForm({ ...form, rtpMin: parseInt(e.target.value) })}
                            className="w-full bg-slate-700/50 border border-slate-600 rounded-xl px-4 py-3 text-white focus:border-blue-500 outline-none"
                            min={0}
                            max={100}
                        />
                    ...
                    <div>
                        ...
                        <input
                            type="number"
                            value={form.rtpMax}
                            onChange={(e) => setForm({ ...form, rtpMax: parseInt(e.target.value) })}
                            className="w-full bg-slate-700/50 border border-slate-600 rounded-xl px-4 py-3 text-white focus:border-blue-500 outline-none"
                            min={0}
                            max={100}
                        />
                ...
                <div className="bg-slate-700/30 rounded-xl p-4">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex-1 bg-slate-600 rounded-full h-3 overflow-hidden">
                            <div
                                className="bg-gradient-to-r from-red-500 via-red-500 to-red-500 h-full"
                                style={{ width: `${form.rtpMax}%`, marginLeft: `${form.rtpMin}%` }}
                            />
                        </div>
                        <span className="text-white font-mono text-sm">
                            {form.rtpMin}% - {form.rtpMax}%
                        </span>
                    </div>
                </div>
                ...
                <div className="flex gap-4 pt-4">
                    <Link
                        href="/admin/games"
                        className="flex-1 py-3 border border-slate-600 rounded-xl text-center text-slate-300 hover:bg-slate-700 transition-colors"
                    >
                        Cancel
                    </Link>
                    <button
                        ...
                        className="flex-1 py-3 bg-blue-600 rounded-xl text-white font-bold hover:bg-blue-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
